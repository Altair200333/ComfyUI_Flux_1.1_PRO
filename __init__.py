...
class FluxProInpaint:
    ...
    def _tensor_to_pil(self, tensor, mode="RGB"):
        arr = np.clip(255.0 * tensor.cpu().numpy().squeeze(), 0, 255).astype(np.uint8)
        return Image.fromarray(arr, mode=mode)

    def _pil_to_base64(self, pil_img, fmt="PNG"):
        buf = io.BytesIO()
        pil_img.save(buf, format=fmt)
        return base64.b64encode(buf.getvalue()).decode("utf-8")

    def _mask_to_base64(self, mask_tensor, expected_size):
        ...
        return self._pil_to_base64(pil_mask)
    ...
    def _make_request(self, method, url, **kwargs):
        try:
            ...
            raise RuntimeError(
                f"[FLUX INPAINT] Network error during {method} request: {e}"
            )
    ...
    def process(
        ...
        try:
            ...
            pil_image = self._tensor_to_pil(
                image, mode="RGBA" if preserve_alpha else "RGB"
            )
            ...
            mask_base64 = None
            if mask is not None:
                mask_base64 = self._mask_to_base64(mask[0], pil_image.size)
            ...
            try:
                ...
                raise RuntimeError(f"[FLUX INPAINT] Network Error: {re}")
    ...
    def get_result(self, task_id, output_format, preserve_alpha=False, max_attempts=15):
        ...
        while attempt <= max_attempts:
            ...
            try:
                resp = self._make_request("GET", get_url, headers=headers, timeout=30)
            except RuntimeError as e:
                print(f"[FLUX INPAINT] Network error during result poll: {e}")
                attempt += 1
                continue
            ...
            if resp.status_code == 200:
                ...
                if status == Status.READY.value:
                    ...
                    try:
                        img_resp = self._make_request("GET", sample_url, timeout=30)
                        ...
                        final_tensor = self._pil_to_tensor(pil_img)
                    except Exception as e:
